...
<aside
            ...
            <nav class="space-y-1.5 px-3">
                @if(in_array(auth()->user()->Role, ['Owner/Admin', 'Inventory Clerk']))
                    <a href="{{ route('dashboard') }}"
                        class="{{ request()->routeIs('dashboard') ? 'bg-orange-600/10 text-orange-400' : 'text-stone-300 hover:text-white hover:bg-stone-800/80' }} group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all relative overflow-hidden"
                        title="{{ auth()->user()->Role === 'Owner/Admin' ? 'Manager Dashboard' : 'Clerk Dashboard' }}">
                        <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('dashboard') ? 'text-orange-400' : 'opacity-70 group-hover:opacity-100 group-hover:text-orange-400 transition-colors' }}"
                ...
                @if(in_array(auth()->user()->Role, ['Owner/Admin', 'Cashier']))
                    <a href="{{ route('pos.index') }}"
                        class="{{ request()->routeIs('pos.*') ? 'bg-orange-600/10 text-orange-400' : 'text-stone-300 hover:text-white hover:bg-stone-800/80' }} group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all relative overflow-hidden"
                        title="Employee Dashboard">
                        <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('pos.*') ? 'text-orange-400' : 'opacity-70 group-hover:opacity-100 group-hover:text-orange-400 transition-colors' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div>
                ...
                <nav class="space-y-1.5 px-3">
                    <a href="{{ route('inventory.index') }}"
                        class="{{ request()->routeIs('inventory.*') ? 'bg-orange-600/10 text-orange-400' : 'text-stone-300 hover:text-white hover:bg-stone-800/80' }} group flex items-center justify-between px-3 py-2.5 text-sm font-medium rounded-lg transition-all relative overflow-hidden"
                        title="Smartphone Inventory">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('inventory.*') ? 'text-orange-400' : 'opacity-70 group-hover:opacity-100 group-hover:text-orange-400 transition-colors' }}"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if(auth()->user()->Role === 'Owner/Admin')
                        <a href="{{ route('reports.index') }}"
                            class="{{ request()->routeIs('reports.*') ? 'bg-orange-600/10 text-orange-400' : 'text-stone-300 hover:text-white hover:bg-stone-800/80' }} group flex items-center justify-between px-3 py-2.5 text-sm font-medium rounded-lg transition-all relative overflow-hidden"
                            title="Reports">
                            <div class="flex items-center">
                                <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('reports.*') ? 'text-orange-400' : 'opacity-70 group-hover:opacity-100 group-hover:text-orange-400 transition-colors' }}"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div>
                ...
                <nav class="space-y-1.5 px-3">
                    <a href="{{ route('users.index') }}"
                        class="{{ request()->routeIs('users.*') ? 'bg-orange-600/10 text-orange-400' : 'text-stone-300 hover:text-white hover:bg-stone-800/80' }} group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all relative overflow-hidden"
                        title="User Accounts">
                        <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('users.*') ? 'text-orange-400' : 'opacity-70 group-hover:opacity-100 group-hover:text-orange-400 transition-colors' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="p-4 border-t border-white/5 flex items-center shrink-0">
        ...
        <div class="ml-3 transition-all duration-150 overflow-hidden"
            ...
            <p class="text-[10px] text-stone-400 truncate">{{ auth()->check() ? auth()->user()->Role : 'System Admin' }}
            </p>
        </div>
    </div>
</aside>